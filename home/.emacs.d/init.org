...
#+begin_src emacs-lisp
  ...
  (defun my-svin-open-review ()
    (interactive)
    (shell-command (concat "svin review --open -r" user-login-name))) ; user-login-name works around a current bug here, and can go away soon.
  ...
  (defun my-svin-commit-review ()
    (interactive)
    (with-editor (compile (concat "svin commit --dry-run -r " (my-svin-review-id)))))
