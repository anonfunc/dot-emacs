...
#+begin_src emacs-lisp
  ...
  (defun my-toggle-frame-fullscreen (frame)
    "toggle-frame-fullscreen with a frame argument"
  ...
  (add-to-list 'after-make-frame-functions 'my-toggle-frame-fullscreen)
...
#+begin_src emacs-lisp
  (when (eq system-type 'darwin)
    ...
    :family "Fira Code Medium" :height 145 :weight 'medium)
...
** Jump to last change
#+begin_src emacs-lisp
  (ensure-pkgs '(smartrep goto-last-change))
  (require 'smartrep)
  (global-set-key (kbd "C-c k") 'goto-last-change)
  (smartrep-define-key
      global-map "C-c"
    '(("k" . goto-last-change)))  ; "C-c k" for searches
#+end_src
...
** Fixmee
#+begin_src emacs-lisp
  (ensure-pkgs '(button-lock fixmee))
  (require 'button-lock)
  (global-fixmee-mode 1)
#+end_src

** Projectile/Fixmee Hydra
#+begin_src emacs-lisp
...
** Golang
#+begin_src emacs-lisp
  (ensure-pkgs '(go-mode go-autocomplete go-eldoc flycheck go-rename))
